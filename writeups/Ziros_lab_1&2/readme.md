...
This challenge was my favorite one because of the story, that was created in him.

## Ziro's Lab 1

!["chall_head"](./lfi_enonce.PNG)

This challenge contained 4 pages, 2 that didn't have any use (index.php and page2.php) and 2 others had one (login.php and page1.php).

Let's firstly focus on `page1.php`. 

!["lfi"](./lfi.PNG)

To access page2, the website used page1. This pages was designed with a parameter :

```
http://10.0.0.4:8080/page1.php?page=page2.php
```

We can see that this page contains an url that includes a file. This type of vulnerabilities are called LFI (Local File Injection). This vulnerability allows an attacker to see the source code of a page that he normally couldn't.

Let's try to include `login.php`, this gives the url
```
http://10.0.0.4:8080/page1.php?page=php://filter/read=convert.base64-encode/resource=login.php
```

This type of LFI uses the wrapper php, which can be used to transform a file in base 64.

!["lfi3"](./lfi3.PNG)

The response gives this base 64:
```
...
This php code reveals a user and a password
```
...
The page was the same as the one we got earlier, except that the querry we did earlier to have the source code of login.php was stopped beacause of a new filter.

To see what was filtered, I firestly retrieved the source code of page1.php.
...
And here wa got the flag another time.